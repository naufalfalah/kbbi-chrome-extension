...
Satu-satunya koneksi jaringan yang dibuat oleh ekstensi ini adalah permintaan HTTP GET ke:
...
• host_permissions (kbbi.kemendikdasmen.go.id) — untuk melewati batasan CORS saat mengambil definisi.
...
2. Network Usage
The only network connection made by this extension is an HTTP GET request to:
...
• host_permissions (kbbi.kemendikdasmen.go.id) — to bypass CORS restrictions when fetching definitions.
